...
namespace SHAH.Controllers
{
    ...
    public class BidController : Controller
    {
        ...
        public async Task<IActionResult> PlaceBid(ItemVM model)
        {
            ...
            var extendedTo = AuctionTimeExtension.ExtendIfNeeded(auction);
            if (extendedTo != null)
            {
                await _unitOfWork.CompleteAsync();
            ...
            if (highestBid != null && highestBid.UserId != userId)
            {
                await _unitOfWork.Notifications.AddAsync(new Notification
                {
                    ...
                    Message = $"You've been outbid on \"{item.Name}\" by ${bidAmount:N2}",
        ...
        public async Task<IActionResult> GetHighest(int itemId)
        {
            ...
            var nextMin = highest + BiddingEngine.CalculateIncrement(highest);
